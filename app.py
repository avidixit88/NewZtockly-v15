...
def can_alert(symbol: str, tier: str, now_ts: float, cooldown_min: int) -> bool:
    last = st.session_state.last_alert_ts.get(f"{symbol}:{tier}")
    if last is None:
        return True
...
def render_alerts_panel():
    ...
    with left:
        ...
        for a in alerts[:30]:
            ...
            title = f"{badge} **{a['symbol']}** — **{a['bias']}** ({a.get('tier')}) — Score **{a['score']}** ({a.get('session','')}) • {pro_badge}"
...
with tab_scan:
    ...
    if results:
        ...
        df = pd.DataFrame([{
            ...
            "Actionable": (r.bias in ["LONG", "SHORT"] and getattr(r, "tier", "NONE") in ["PRE","CONFIRMED"]),
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Actionable": st.column_config.CheckboxColumn("Actionable", help="True when the engine can produce Entry/Stop/TP (PRE or CONFIRMED)."),
        ...
        tier_top = getattr(top, "tier", "NONE")
        st.success(f"Top setup: **{top.symbol}** — **{top.bias}** ({tier_top}, Score {top.setup_score}, {top.session}) • {pro_badge}")
        ...
        for r in results:
            ...
            tier_now = getattr(r, "tier", "NONE")
            tier_changed = (prev_tier != tier_now)
            ...
            if not (crossed_threshold or became_actionable or flipped_direction or tier_changed):
                continue

            if not can_alert(r.symbol, getattr(r, "tier", "NONE"), now, cooldown_minutes):
            ...
            st.session_state.last_alert_ts[f"{r.symbol}:{tier_now}"] = now
